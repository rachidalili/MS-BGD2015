...
def informationRecommandation(text):
	informations = text.findAll(['a', 'li'])
	return informations[0].text.encode('utf-8').split(' - '), informations[1].text.encode('utf-8'), informations[2].text.encode('utf-8'), informations[3].text.encode('utf-8')


for node in soup.findAll("div", {"class":"yt-lockup-content"}):

		print informationRecommandation(node)
		infs = informationRecommandation(node)
		print type(infs)
		print infs[0]
